...
```
启动 → 查 DB 获取身份 → 启动 30 分钟超时计时器

每条消息：
  1. 重置超时计时器
  2. 检索记忆（必选池 + 动态池）
  3. 注入 Prompt（基础人设 + 关系层 + 记忆层）
  4. LLM 流式回复
  5. push 到 history（给 LLM）+ unextractedBuffer（给记忆提取）
  6. 每 5 轮 → 增量提取（从 buffer 取 → 写 DB → 清空 buffer → 压缩上下文摘要）

会话结束（Ctrl+C / 超时 30 分钟）：
  1. 保存 buffer 快照
  2. 兜底提取剩余记忆（清空 buffer）
  3. 生成对话摘要（基于压缩摘要 + 快照，不用全量 history）
```

**关键设计决策**：
- 用独立 `unextractedBuffer` 而非 history 下标追踪未提取消息（防止下标错位）
- 对话摘要基于"压缩摘要 + 最后未提取部分"，不用全量 history（token 可控）
- 上下文摘要超过 1500 字时 LLM 压缩（不硬截断）
