...
// 外界只需要传平台身份、消息，以及可选的自定义人格指令
const result = await getChatResponse({
  platformId: 'zimu',
  platform: 'system',
  customDirective: '和我说话可以更放松一点，少一点客套，多一点亲近感。',
...
2. 实现模型注册（`core/src/ai/models.ts`）
   - `sao10k/l3.3-euryale-70b`（OpenRouter）作为主对话模型
...
## Batch 7：用户自定义 + 完整循环 ✅ 已完成（2026-04-10）

### 目标
M1 核心完成。完整循环跑通。

### 任务
1. 实现用户自定义配置读取（显式输入，不做聊天内自动学习）
2. 更新 Prompt Composer：按需注入自定义层
