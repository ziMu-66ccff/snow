...
这些数字由 Embedding 模型（`baai/bge-m3`，通过 OpenRouter 调用）生成，模型"理解"了文本的语义。
...
const { embedding } = await embed({
  model: openrouter.embeddingModel('baai/bge-m3'),
  value: memoryText,
});
// embedding = [0.12, -0.34, 0.78, ...] （1024 维向量）
...
const result = streamText({
  model: openrouter.chat('sao10k/l3.3-euryale-70b'),
...
| Embedding | **`baai/bge-m3` via OpenRouter** | 1024 维，向量化记忆用 |
...
| 场景 | 需要什么能力 | 推荐模型 | 成本 |
|------|-------------|---------|------|
| **核心对话**（带性格、带情感） | 角色扮演、情感理解、创造力 | **Llama 3.3 Euryale 70B**（OpenRouter） | 中 |
| **结构化任务**（情绪、记忆、关系、趋势摘要） | 结构化输出、信息抽取、低成本高频调用 | **Gemini 2.5 Flash-Lite** | 低 |
| **复杂推理/困难对话**（心理疏导等） | 深度推理、同理心 | **DeepSeek Reasoner** / 高端模型 | 高 |
| **轻量任务**（天气查询、简单问答） | 基础能力即可 | **DeepSeek Chat** / 轻量模型 | 极低 |

##### 当前推荐的三层策略

1. **主对话层**：`sao10k/l3.3-euryale-70b`
   - 用于 Snow 的核心陪伴对话、人设表达、关系温度和角色氛围
   - 重点是“像 Snow 一样说话”，而不是最低成本

2. **结构化任务层**：`Gemini 2.5 Flash-Lite`
   - 用于情绪分析、记忆提取、关系信号分析、趋势摘要等结构化任务
   - 重点是低成本、高频、结构稳定

3. **辅助 / 备用层**：`DeepSeek`
   - `deepseek-chat`：低成本一般任务
   - `deepseek-reasoner`：复杂推理、困难对话兜底

这套分层的核心原则是：
- 让最有“人味”的模型负责主对话
- 让最稳、最便宜的结构化模型承担后台高频任务
- 让 DeepSeek 作为低成本辅助和复杂推理备用
...
// Gemini 也走 OpenAI-compatible provider 时，可统一挂在 createOpenAI 上
...
const models = {
  chat: openrouter.chat('sao10k/l3.3-euryale-70b'),
  structured: gemini.chat('gemini-2.5-flash-lite'),
  cheap: deepseek.chat('deepseek-chat'),
  reasoning: deepseek.chat('deepseek-reasoner'),
  embedding: openrouter.embeddingModel('baai/bge-m3'),
};

function selectModel(task: 'chat' | 'structured' | 'cheap' | 'reasoning') {
  return models[task];
}
```

> **当前口径**：主对话和结构化任务已经明确分层，不再是“先全部用 DeepSeek 跑通”。
...
├─────────────────────────────────────────────────────┤
│  AI 模型                                             │
│  Llama 3.3 Euryale 70B (主对话)                       │
│  Gemini 2.5 Flash-Lite (结构化任务)                   │
│  DeepSeek Chat / Reasoner (辅助与兜底)                │
│  BAAI bge-m3 via OpenRouter (向量化，1024 维)          │
├─────────────────────────────────────────────────────┤
│  数据存储                                            │
...
| Next.js 15 | ✅ | 前端 + API |
| Vercel AI SDK | ✅ | 流式对话 |
| `sao10k/l3.3-euryale-70b` | ✅ | 主对话模型 |
| Gemini 2.5 Flash-Lite | ✅ | 结构化任务模型 |
| DeepSeek Chat / Reasoner | ✅ | 辅助与兜底 |
| Supabase PG | ✅ | 用户、记忆、关系 |
| pgvector | ✅ | 记忆语义检索 |
| OpenRouter `baai/bge-m3` | ✅ | 记忆向量化（1024 维） |
| Drizzle ORM | ✅ | 数据库访问 |
| Tailwind + shadcn | ✅ | UI |
...
| 2 | Embedding 方案 | ✅ `baai/bge-m3` via OpenRouter（1024 维） |
| 3 | Supabase 项目 | ✅ 已创建 |
| 4 | Vercel 账号 | ✅ 有，先用免费版 |
...
│       ↓                                              │
│  Vercel AI SDK → 编排 Prompt → 调用 Euryale → 流式回复 │
│       ↓                                              │
...
| **Llama 3.3 Euryale 70B** | Snow 的“主说话层”——把人格、关系和情绪真正说出来 |
| **Gemini 2.5 Flash-Lite** | Snow 的“结构化分析层”——低成本处理情绪、记忆、关系与趋势 |
| **DeepSeek** | Snow 的“低成本辅助层”——一般任务和复杂推理兜底 |
...
| 2026-04-09 | v0.5 | 当前实现口径同步：Embedding 改为 `baai/bge-m3` via OpenRouter，向量维度统一为 1024 |
| 2026-04-10 | v0.6 | 模型策略更新：主对话改为 `sao10k/l3.3-euryale-70b`，结构化任务采用 Gemini 2.5 Flash-Lite，DeepSeek 作为辅助与兜底 |
